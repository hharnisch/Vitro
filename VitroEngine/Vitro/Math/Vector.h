...
namespace Vitro
{
	...
	constexpr auto Length(const Vector<D, N>& vec) -> decltype(std::sqrt(vec.X))
	{
		return std::sqrt(Dot(vec, vec));
	}

	template<size_t D, typename N>
	constexpr auto Normalize(const Vector<D, N>& vec) -> Vector<D, decltype(Length(vec))>
	{
	...
	template<size_t D, typename N>
	constexpr auto Sqrt(const Vector<D, N>& vec) -> Vector<D, decltype(std::sqrt(vec.X))>
	{
		return Apply(vec, static_cast<decltype(std::sqrt(vec.X))(*)(N)>(std::sqrt));
	}

	template<size_t D, typename N>
	constexpr auto InvSqrt(const Vector<D, N>& vec) -> decltype(Sqrt(vec))
	{
		return 1 / Sqrt(vec);
	}
	...
	template<size_t D, typename N>
	constexpr auto Sin(const Vector<D, N>& vec) -> Vector<D, decltype(std::sin(vec.X))>
	{
		return Apply(vec, static_cast<decltype(std::sin(vec.X))(*)(N)>(std::sin));
	}

	template<size_t D, typename N>
	constexpr auto Cos(const Vector<D, N>& vec) -> Vector<D, decltype(std::cos(vec.X))>
	{
