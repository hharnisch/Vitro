...
namespace Vitro
{
	...
	public:
		...
		template<typename O0, typename O1, typename O2, typename O3, typename O4, typename O5,
			...
			constexpr Matrix(O0 x0, O1 y0,
							 O2 x1, O3 y1,
							 O4 x2, O5 y2) :
			Raw {Col(x0, y0), Col(x1, y1), Col(x2, y2)}
		{}

		template<typename O0, typename O1, typename O2, typename O3, typename O4, typename O5,
			typename O6, typename O7, typename O8, typename std::enable_if_t<R == 3, O0>* = nullptr>
		...
		constexpr Matrix(O scalar) : Raw {Col(scalar), Col(scalar), Col(scalar)} {}
		...
		{
			Raw[0] += m[0]; Raw[1] += m[1]; Raw[2] += m[2]; return *this;
		}
		...
		{
			Raw[0] += scalar; Raw[1] += scalar; Raw[2] += scalar; return *this;
		}
	...
	template<typename N, typename O>
	constexpr Vector<2, N> operator*(const Matrix<2, 3, N>& m, const Vector<3, O>& v)
	{
	...
	template<typename N, typename O>
	constexpr Vector<3, N> operator*(const Vector<2, O>& v, const Matrix<2, 3, N>& m)
	{
	...
	template<typename N, typename O>
	constexpr Vector<3, N> operator*(const Matrix<3, 3, N>& m, const Vector<3, O>& v)
	{
		return {m[0].X * v.X + m[1].X * v.Y + m[2].X * v.Z,
				m[0].Y * v.X + m[1].Y * v.Y + m[2].Y * v.Z,
	...
	template<typename N, typename O>
	constexpr Vector<3, N> operator*(const Vector<3, O>& v, const Matrix<3, 3, N>& m)
	{
	...
	template<typename N, typename O>
	constexpr Vector<4, N> operator*(const Matrix<4, 3, N>& m, const Vector<3, O>& v)
	{
		return {m[0].X * v.X + m[1].X * v.Y + m[2].X * v.Z,
				m[0].Y * v.X + m[1].Y * v.Y + m[2].Y * v.Z,
				m[0].Z * v.X + m[1].Z * v.Y + m[2].Z * v.Z,
	...
	template<typename N, typename O>
	constexpr Vector<3, N> operator*(const Vector<4, O>& v, const Matrix<4, 3, N>& m)
	{
	...
	constexpr Matrix<3, 3, F> Inverse(const Matrix<3, 3, N>& m)
	{
