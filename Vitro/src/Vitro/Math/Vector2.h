...
namespace Vitro
{
	...
	template<typename N> struct Vector<2, N>
	{
		...
		template<typename O> constexpr Vector(O scalar) :
			Vector(static_cast<N>(scalar), static_cast<N>(scalar))
		{}

		template<size_t D, typename O> constexpr Vector(const Vector<D, O>& other) :
		...
		template<typename O, VTR_IS_SCALAR(O)> inline Vector<2, N> operator+(O scalar) const
		{
		...
		template<typename O, VTR_IS_SCALAR(O)> inline Vector<2, N>& operator+=(O scalar)
		{
